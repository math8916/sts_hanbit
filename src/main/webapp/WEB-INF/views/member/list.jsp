...
<ul class="list-group">
  ...
  <li class="list-group-item">총학생수 :${total}명</li>
</ul>
...
<table id="member_list_table">
  ...
  <tr>
  	<td>${member.id}</td>
  	<td><a class="name">${member.name}</td>
...
<nav aria-label="Page navigation">
  ...
  <c:if test="${startPg-pgSize gt 0 }">
  	<li>
  	<a href ="${context}/member/list${startPg-pgSize}" arial-label="Previous" }>
    ...
    <c:if test="${startPg+pgSize le totPg }">
  	<li>
  	<a href ="${context}/member/list${startPg-pgSize}" arial-label="Next" }>
           <span aria-hidden="true">&laquo;</span>
      </a>
    </li>
    </c:if>
